...
class TrajectoryClient(Node):
    def __init__(self):
        ...
        self.trajectory_client = ActionClient(self, TrajectoryGoal, "/centauri/trajectory")

        self.gripper_client = self.create_client(SetBool, "/gripper")
        while not self.gripper_client.wait_for_service(timeout_sec= 5.0):
            print("\nWaiting for an available gripper service")
        self.srv_request = SetBool.Request()

        print("\nInitializing trajectory client")
    
    def send_goal(self, des_goal, time_range):
        ...
        status_future = self.trajectory_client.send_goal_async(goal_msg, self.get_goal_feedback)
    ...
    def get_goal_result(self, result_future_msg):
        ...
        print(f"\nGot to θ: {np.around(np.rad2deg(q_final), 2)} °")
...
def main():
    ...
    # Goal Example 1: 0.351 -0.288 0.351 5.0
    # Goal Example 2: 0.161 -0.288 0.547 3.5
    # Goal Example 3: 0.351  0.001 0.405 4.5
    ...
    try:
        ...
        print(f"\nGet to {des_goal} m in {time_range} seconds")
        node.send_goal(des_goal, time_range)
        rclpy.spin(node)
