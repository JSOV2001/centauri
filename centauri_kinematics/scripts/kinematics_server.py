...
class KinematicsServer(Node):
    def __init__(self):
        ...
        self.e_max = 0.1/100
        self.kinematics_server = self.create_service(KinematicGoal, "/centauri/kinematics", self.kinematics_callback)
        ...
        self.controller_pub = self.create_publisher(Float64MultiArray, self.controller_topic, 1)

        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)
        self.source_frame = "base_link"
        self.target_frame = "tool_center"
    ...
    def kinematics_callback(self, request, response):
        ...
        if self.is_controller_enabled:
            self.controller_msg.data = q_solv.tolist()
    ...
    def get_tf(self):
        ...
        rate.sleep()
        self.tf_curr = self.tf_buffer.lookup_transform(
            self.source_frame,
            self.target_frame, 
            rclpy.time.Time(),
            rclpy.duration.Duration(seconds=2.5)
        )
        return self.tf_curr
