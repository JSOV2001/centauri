...
class KinematicsServer(Node):
    ...
    def kinematics_callback(self, request, response):
        ...
        if success:
            ...
            # Publish soluction in Gazebo's controller
            for q_curr in self.q_list:
                controller_data.append(q_curr)

            self.q_prev = self.q_list.copy().astype(float)
        else:
            for q_curr in self.q_prev:
                controller_data.append(q_curr)
        
        self.controller_msg.data = controller_data
        self.controller_pub.publish(self.controller_msg)
        
        # Define server's response to client
        response.q_list = controller_data
